...
def main():
    ...
    logger.info("=" * 60)
    logger.info("📋 多平台发布 - 2026/07/08 批次4 重发v4（第7-16篇）")
    logger.info(f"   共 {len(posts)} 篇，3平台")
    logger.info(f"   间隔 {settings.multipost_post_interval} 秒（±30秒抖动）")
    logger.info("=" * 60)

    for i, p in enumerate(posts, 7):
        logger.info(f"  [{i:2d}] {p['topic'][:25]}... ({len(p['body'])}c)")
